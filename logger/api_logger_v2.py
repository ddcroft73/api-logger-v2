...
'''
    ...
    The Stream class is just a way to organize the log streams, and their prefixes that are used when 
    making a log enty.

'''
class Stream:   
    INFO: int = 10                #   Anything, really basic info that needs to be logged thats not serious.. heads up
    DEBUG: int = 20               #   To be used during development. A place to log all the debugging temporary info. 
    ERROR: int = 30               #   Pretty self explanatory here, but.. Exception feedback, etc
...
class APILogger_v2():   
    # Going to attempt to see If I can keep thes in the Settings class once again.
    LOG_DIRECTORY: str = settings.LOG_DIRECTORY                   #'./logs'
    LOG_ARCHIVE_DIRECTORY: str = settings.LOG_ARCHIVE_DIRECTORY  # f"{LOG_DIRECTORY}/log-archives"
    DEFAULT_LOG_FILE: str = settings.DEFAULT_LOG_FILE           #f"{LOG_DIRECTORY}/DEFAULT-app-logs.log"  

    USERS_DIRECTORY: str = settings.USERS_DIRECTORY
    SYSTEM_DIRECTORY: str = settings.SYSTEM_DIRECTORY  # Not sure about this.
    ...
    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            cls._instance = super(APILogger_v2, cls).__new__(cls)
    ...
    def __init__(self, *args, **kwargs):
        if not self._initialized:
            ...
            # instantiate all objects needed to run the logger...
            self.stream = Stream()
            self.archive = Archive(
                archive_directory=APILogger_v2.LOG_ARCHIVE_DIRECTORY
            )
    ...
    def __handle_file_setup(self) -> None:
        ...
        this does not have to be async... 
        """
        if self.archive_log_files:
            self.archive.set_archive_directory(APILogger_v2.LOG_ARCHIVE_DIRECTORY)
        ...
        # When supporting multiple users, we will first loop through the users and
        #  for user in users:
        #       # for this user create a directory in the logs
        #        logs/users/{thisuser}/file_name 


        for file_name in file_names:
            if file_name is not None:
                self.__set_log_filename(os_join(APILogger_v2.LOG_DIRECTORY, file_name))

        if any(filename is None for filename in file_names):
            self.__set_log_filename(APILogger_v2.DEFAULT_LOG_FILE)
    ...
    def __save_log_entry(
        ...
        def prep_N_format(file_name: str, message: str):
            ...
            if file_name is None or file_name == "None":
                file_name = APILogger_v2.DEFAULT_LOG_FILE
            else:
                file_name = os_join(APILogger_v2.LOG_DIRECTORY, file_name)
